(function () {
    ...
    fetchCompanyData(function (fetchedPlacement) {
        ...
        let percentageParagraph = null
        let referralParagraph = null
        let saleParagraph = null
        let promotionParagraph = null

        if (placement.payout_type && placement.payout_type === 'revenue_share' && placement.payout_decimal > 0) {
           percentageParagraph = buildParagraphWithSpan(placement, 'percentage')
        }
        ...
        if (placement.active_promotion && placement.active_promotion.title) {
            promotionParagraph = buildParagraphWithSpan(placement, 'promotion')
            const promotionTooltip = document.createElement('div')
            promotionTooltip.classList.add('tooltip')
            promotionTooltip.id = 'sb-promotion-payout-tooltip'
            promotionTooltip.textContent = placement.active_promotion.title
            promotionTooltip.setAttribute('role', 'tooltip')
            promotionParagraph.append(promotionTooltip)
        }   
        ...
        if (placement.sale_payout_tooltip && (saleParagraph || percentageParagraph)) {
            const salePayoutTooltip = buildPayoutTooltip(placement, 'sale')
            if (saleParagraph) {
                saleParagraph.append(salePayoutTooltip)
            } else if (percentageParagraph) {
                percentageParagraph.append(salePayoutTooltip)
            }
        }
       

        let payoutInfo = null
        if (referralParagraph || saleParagraph || promotionParagraph || percentageParagraph) {
            payoutInfo = buildPayoutInfo(referralParagraph, saleParagraph, promotionParagraph, percentageParagraph)
        }
        ...
        if (placement.active_promotion && placement.active_promotion.title) {
            const promoButton = document.querySelector('#sb-promotion-tooltip-button')
            const promoTooltip = document.querySelector('#sb-promotion-payout-tooltip')
            if (promoButton && promoTooltip) {
                addEventListeners(promoButton, promoTooltip)
            }
        }
    ...
    function buildParagraphWithSpan(placement, type) {
        ...
        if (type === 'referral' || type === 'sale') {
            ...
            paragraphElement.style.paddingLeft = '16px'
            paragraphElement.style.paddingRight = '16px'

            paragraphElement.appendChild(document.createTextNode('You get '))

            const spanElement = document.createElement('span')
            spanElement.classList.add('sb-amount')
            spanElement.textContent = `${placement.payout_decimal * 100}%`
            paragraphElement.appendChild(spanElement)

            paragraphElement.appendChild(document.createTextNode(' of revenue from each '))

            if (placement.sale_payout_tooltip) {
                const tooltipButton = document.createElement('button')
                tooltipButton.classList.add('tooltip-button')
                tooltipButton.id = 'sb-sale-tooltip-button'

                const buttonTextSpan = document.createElement('span')
                buttonTextSpan.classList.add('tooltip-button-text')
                buttonTextSpan.textContent = 'qualified sale'
                ...
                paragraphElement.appendChild(document.createTextNode('qualified sale'))
            }
        }   
    ...
    function buildPayoutInfo(referralParagraph, saleParagraph, promotionParagraph, percentageParagraph) {
        const payoutInfo = document.createElement('div')
        payoutInfo.id = 'sb-payout-info-container'

        if (percentageParagraph) {
            payoutInfo.appendChild(percentageParagraph)
        }
        ...
        if (promotionParagraph) {
            payoutInfo.appendChild(promotionParagraph)
        }
